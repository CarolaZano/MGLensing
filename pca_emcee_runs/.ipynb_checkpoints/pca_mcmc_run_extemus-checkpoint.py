...
MGL_mu_lin = MGLensing.MGL("ini_files/pca/config_muSigma_lin_PCA_ext.yaml")
MGL_GR_nl = MGLensing.MGL("ini_files/pca/config_GR.yaml")
MGL_GR_lin = MGLensing.MGL("ini_files/pca/config_GR_lin.yaml")
MGL_nDGP_nl = MGLensing.MGL("ini_files/pca/config_nDGP_ext.yaml")
MGL_nDGP_lin = MGLensing.MGL("ini_files/pca/config_nDGP_ext_lin.yaml")
MGL_fR_nl = MGLensing.MGL("ini_files/pca/config_fR_ext.yaml")
MGL_fR_lin = MGLensing.MGL("ini_files/pca/config_fR_ext_lin.yaml")
...
def initialize_emulators():
    ...
    pars = {}
    for par in MGL_mu_lin.params_model:
        prior_info = MGL_mu_lin.params_priors[par]
        if prior_info['type'] == 'G':
            pars[par] = prior_info['p1']
        elif prior_info['type'] == 'U':
            pars[par] = 0.5 * (prior_info['p1'] + prior_info['p2'])
    # Use fiducial values for initialization
    param_dic = pars | MGL_mu_lin.params_fixed.copy()
    param_dic_all, status = MGL_fR_lin.Like.Theo.check_pars(param_dic)
    if status:
        print("Initializing emulators with fiducial params...")
        MGL_fR_lin.Like.compute_data_vector(param_dic_all)
        MGL_nDGP_nl.Like.compute_data_vector(param_dic_all)
        MGL_fR_nl.Like.compute_data_vector(param_dic_all)
        MGL_nDGP_lin.Like.compute_data_vector(param_dic_all)
        print("Emulators initialized.")
    else:
        raise RuntimeError("Initialization parameters are not valid.")

def log_probability_function(pars):
        param_dic = pars | MGL_mu_lin.params_fixed
        param_dic_all, status = MGL_fR_lin.Like.Theo.check_pars(param_dic)
        
        if status and (0.04 <= param_dic_all["Omega_b"] <= 0.06):
...
for par_i in MGL_mu_lin.params_model:    
    ...
    #elif MGL_mu_lin.params_priors[par_i]['type'] == 'G&U':
    #    prior.add_parameter(par_i,
    #    dist=truncnorm((MGL_mu_lin.params_priors[par_i]['p3'] - MGL_mu_lin.params_priors[par_i]['p1']) / MGL_mu_lin.params_priors[par_i]['p2'],
    #              (MGL_mu_lin.params_priors[par_i]['p4'] - MGL_mu_lin.params_priors[par_i]['p1']) / MGL_mu_lin.params_priors[par_i]['p2'],
    #              loc=MGL_mu_lin.params_priors[par_i]['p1'],
    #              scale=MGL_mu_lin.params_priors[par_i]['p2']))
